  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Guarde dinheiro para o que importa.
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {q.isError ? (
        ...
        >
          Não foi possível carregar as metas. Tente atualizar a página.
        </p>
      ) : q.isLoading ? (
  ...
  return (
    <li className="rounded-2xl border border-border bg-card p-5">
      ...
      <form onSubmit={handleAdd} className="mt-4 flex gap-2">
        <label htmlFor={`add-${goal.id}`} className="sr-only">
...
function GoalDialog({
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!name.trim() || !Number.isFinite(t) || t <= 0) {
      toast.error("Informe nome e valor da meta.");
      return;
    }
    if (!Number.isFinite(c) || c < 0) {
      toast.error("O valor já guardado não pode ser negativo.");
      return;
    }
    setSaving(true);
    try {
