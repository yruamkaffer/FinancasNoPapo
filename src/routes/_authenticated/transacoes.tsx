...
function TransactionsPage() {
  ...
  }, [q.data, filterType, filterCat, search]);
  ...
  return (
    <div className="space-y-5">
      ...
      {q.isError ? (
        ...
        >
          Não foi possível carregar as transações. Tente atualizar a página.
        </p>
      ) : q.isLoading ? (
...
function TxRow({ tx, onEdit }: { tx: Tx; onEdit: () => void }) {
  ...
  return (
    <li className="flex items-center gap-3 p-4">
      ...
      <p
        className={`shrink-0 text-sm font-semibold ${tx.type === "income" ? "text-income" : "text-expense"}`}
      >
      ...
      {confirming && (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby={`del-${tx.id}`}
          className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 p-4"
        >
          <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-5 shadow-lg">
            <h2
              id={`del-${tx.id}`}
              className="font-display text-lg text-card-foreground"
            >
              Excluir transação?
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              {tx.description} — {formatBRL(tx.amount)}. Essa ação não pode ser
              desfeita.
            </p>
            <div className="mt-4 flex justify-end gap-2">
              <button
                onClick={() => setConfirming(false)}
                className="rounded-lg border border-input bg-background px-3 py-2 text-sm font-medium hover:bg-secondary"
              >
                Cancelar
              </button>
              <button
    ...
    >
      ...
      >
        <div className="flex items-start justify-between">
          <h2
            id="tx-title"
            className="font-display text-xl text-card-foreground"
          >
