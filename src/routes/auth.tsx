...
function AuthPage() {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Não foi possível entrar com o Google.";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main id="conteudo" className="flex min-h-dvh items-center justify-center bg-background px-4 py-8">
      <div className="w-full max-w-md">
        <Link to="/" className="mb-6 inline-block font-display text-lg text-primary">
          ← Finanças no Papo
        </Link>
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {isSignUp ? "Comece a organizar sua vida financeira." : "Bem-vindo de volta."}
          </p>
          ...
          <form onSubmit={handleEmail} className="space-y-4">
            {isSignUp && (
              <div>
                <label htmlFor="name" className="block text-sm font-medium text-foreground">Como podemos te chamar?</label>
                <input
            ...
            <div>
              ...
              />
              <p id="pwd-hint" className="mt-1 text-xs text-muted-foreground">Mínimo de 6 caracteres.</p>
            </div>
            <button
